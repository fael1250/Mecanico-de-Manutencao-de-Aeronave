...
const Dashboard: React.FC<DashboardProps> = ({ user, currentChapterIndex, setCurrentChapterIndex }) => {
  const { progress, toggleSubTopicComplete, isLoaded } = useProgress(user);
  ...
  if (!isLoaded) {
      return (
        <div className="text-center flex flex-col items-center justify-center h-64">
            ...
            <p className="font-technical text-gray-300">Carregando progresso...</p>
        </div>
    );
  }
